...
root.title("ALzar NFTs a OpenSea v2.0.1 - 2Captcha.com Solución")
...
def supportURL():
    webbrowser.open_new("https://discord.gg/5SfUmknAuV")

def coffeeURL():
    webbrowser.open_new("https://ko-fi.com/obijuan007")

def ethereURL():
    webbrowser.open_new("https://etherscan.io/address/0x30271073D426621f123Ae4b2ac1Db4FaA0F8a6cd")
...
def open_chrome_profile():
    subprocess.Popen(
        [
            ...
            "--user-data-dir=" + main_directory + "/chrome_perfil",
        ],
        shell=True,
    )
...
###input objects###
collection_link_input = InputField("OpenSea enlace de la Coleccion:", 2, 0, 1)
start_num_input = InputField("Numero de Inicio:", 3, 0, 2)
end_num_input = InputField("Numero Final:", 4, 0, 3)
price = InputField("Precio Predeterminado:", 5, 0, 4)
title = InputField("Titulo:", 6, 0, 5)
description = InputField("Descripcion:", 7, 0, 6)
file_format = InputField("NFT Formato de Imagen:", 8, 0, 7)
external_link = InputField("Enlace Externo:", 9, 0, 8)
...
def main_program_loop(prgrm):
    ...
    options.add_experimental_option("debuggerAddress", "localhost:9515")
...
tk.Radiobutton(duration_date, text='1 día', variable=duration_value, value=1, anchor="w", command=save_duration, width=6,).grid(row=0, column=1)
tk.Radiobutton(duration_date, text="3 días", variable=duration_value, value=3, anchor="w", command=save_duration, width=6, ).grid(row=0, column=2)
tk.Radiobutton(duration_date, text="7 días", variable=duration_value, value=7, anchor="w", command=save_duration, width=6,).grid(row=0, column=3)
tk.Radiobutton(duration_date, text="30 días", variable=duration_value, value=30, anchor="w", command=save_duration, width=7,).grid(row=0, column=4)
tk.Radiobutton(duration_date, text="90 días", variable=duration_value, value=90, anchor="w",command=save_duration,  width=7,).grid(row=0,  column=5)
tk.Radiobutton(duration_date, text="180 días", variable=duration_value, value=180, anchor="w", command=save_duration, width=7).grid(row=0, column=6)
duration_date.label = Label(root, text="Duración:", anchor="nw", width=20, height=2 )
...
isSensitive = tkinter.Checkbutton(root, text='Contenido Sensible', var=is_sensitivecontent,   width=49, anchor="w")
isSensitive.grid(row=17, column=1)
isCreate = tkinter.Checkbutton(root, text='Listado Completo', var=is_listing, width=49, anchor="w")
...
upload_folder_input_button = tkinter.Button(root, width=50, height=1,  text="Agregar carpeta de carga de NFTs", command=upload_folder_input)
upload_folder_input_button.grid(row=21, column=1, padx=2)
open_browser = tkinter.Button(root, width=50, height=1,  text="Abrir navegador Chrome", command=open_chrome_profile)
open_browser.grid(row=23, column=1, pady=2)
button_save = tkinter.Button(root, width=50, height=1,  text="Guardar este Formulario", command=save) 
button_save.grid(row=22, column=1, pady=2)
button_start = tkinter.Button(root, width=44, height=2, bg="green", fg="white", text="Iniciar", command=partial(main_program_loop, "Full"))#command=lambda: main_program_loop("Full")
button_start['font'] = font.Font(size=10, weight='bold')
button_start.grid(row=25, column=1, pady=2)

button_collection_scraper = tkinter.Button(root, width=22, height=2, bg="red", fg="white", text="RASPAR Colección", command=collection_scraper)
button_collection_scraper['font'] = font.Font(size=10, weight='bold')
button_collection_scraper.grid(row=27, column=1, padx=0, pady=0)

button_modify_Scrape_txt = tkinter.Button(root, width=22, height=2, bg="orange", fg="black", text="Modificar texto Raspado", command=modify_Scrape_txt)
button_modify_Scrape_txt['font'] = font.Font(size=10, weight='bold')
button_modify_Scrape_txt.grid(row=28, column=1, padx=0, pady=0)

button_onlyListing = tkinter.Button(root, width=44, height=2, bg="#aa5533", fg="white", text="SOLO Listar", command=partial(main_program_loop, "OnlyListing"))
button_onlyListing['font'] = font.Font(size=10, weight='bold')
button_onlyListing.grid(row=29, column=1, pady=2)

footer = tkinter.Button(root, height=3, width=60, text='¿Quieres mostrar apoyo? \n Ahora tienes la oportunidad de comprarme un café. Gracias.',  command=coffeeURL, relief=GROOVE  )
footer.grid(row=31, columnspan=2, padx=31, pady=1)
footer = tkinter.Button(root, height=3, width=80, text='Tambien puedes donarme en cualquier Token de Criptomonedas a la siguiente dirección Ethereum \n 0x30271073D426621f123Ae4b2ac1Db4FaA0F8a6cd',  command=ethereURL, relief=GROOVE  )
footer.grid(row=32, columnspan=2, padx=31, pady=1)

button_onlyListing = tkinter.Button(root, width=44, height=2, bg="#3358FF", fg="white", text="¿Necesitas Ayuda?", command=partial(main_program_loop, "OnlyListing"))
button_onlyListing['font'] = font.Font(size=10, weight='bold')
button_onlyListing.grid(row=33, column=1, pady=2)
